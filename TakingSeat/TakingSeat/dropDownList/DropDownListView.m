...
- (id)initWithFrame:(CGRect)frame dataSource:(id)datasource delegate:(id) delegate
{
    ...
    if (self) {
        ...
        for (int i = 0; i <sectionNum; i++) {
            _mbutton= [[UIButton alloc] initWithFrame:CGRectMake(sectionWidth*i, 1, sectionWidth, frame.size.height-2)];
            _mbutton.tag = SECTION_BTN_TAG_BEGIN + i;
            [_mbutton addTarget:self action:@selector(sectionBtnTouch:) forControlEvents:UIControlEventTouchUpInside];
            _sectionBtnTitle = @"--";
            if ([self.dropDownDataSource respondsToSelector:@selector(titleInSection:index:)]) {
                _sectionBtnTitle = [self.dropDownDataSource titleInSection:i index:[self.dropDownDataSource defaultShowSection:i]];
            }
            [_mbutton setTitle:_sectionBtnTitle forState:UIControlStateNormal];
            [_mbutton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
            _mbutton.titleLabel.font = [UIFont boldSystemFontOfSize:14.0f];
...
-(void)sectionBtnTouch:(UIButton *)btn
{
    ...
    if (_currentExtendSection == section) {
        ...
        currentIV = (UIImageView *)[self viewWithTag:SECTION_IV_TAG_BEGIN + _currentExtendSection];
        [UIView animateWithDuration:0.3 animations:^{
            currentIV.transform = CGAffineTransformRotate(currentIV.transform, DEGREES_TO_RADIANS(180));
        }];
        
        [self showChooseListViewInSection:_currentExtendSection choosedIndex:[self.dropDownDataSource defaultShowSection:_currentExtendSection]];
    }
...
-(void)showChooseListViewInSection:(NSInteger)section choosedIndex:(NSInteger)index
{
    if (!self.mTableView) {
        ...
        self.mTableView.delegate = self.dropDownDelegate;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([self.dropDownDelegate respondsToSelector:@selector(chooseAtSection:index:)]) {
       _chooseCellTitle = [self.dropDownDataSource titleInSection:currentExtendSection index:indexPath.row];
        

        UIButton *currentSectionBtn = (UIButton *)[self viewWithTag:SECTION_BTN_TAG_BEGIN + currentExtendSection];
        [currentSectionBtn setTitle:_chooseCellTitle forState:UIControlStateNormal];
...
{
    return [self.dropDownDataSource numberOfRowsInSection:_currentExtendSection];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [self.dropDownDataSource titleInSection:_currentExtendSection index:indexPath.row];
